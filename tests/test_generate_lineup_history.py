...
from stagediver.cli.generate_lineup_history import (
...
def test_historical_data_handling():
    ...
    assert (
        artist1_records[0]["_valid_from"] == ts1
    ), "Should preserve original valid_from"
    ...
    assert (
        artist1_records[0]["stage_name"] == "Stage A"
    ), "Should preserve original stage"

    # Second record should be current
    assert (
        artist1_records[1]["_valid_from"] == ts1
    ), "Should preserve original valid_from"
...
def test_artist_removal():
    ...
    lineup1 = {
        ...
        "scrape_ts": ts1,
        "artists": [
            {"artist_name": "Artist 1", "artist_id": "id1", "stage_name": "Stage A"},
            {"artist_name": "Artist 2", "artist_id": "id2", "stage_name": "Stage B"},
        ],
    }
    ...
    assert len(artist2_records) == 1, "Artist 2 should have 1 record"
    assert (
        artist2_records[0]["_valid_from"] == ts1
    ), "Should preserve original valid_from"
