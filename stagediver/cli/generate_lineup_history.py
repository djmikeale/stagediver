...
from stagediver.common.config import HISTORICAL_FILE, LINEUPS_FILE
...
def has_relevant_changes(current: Dict, previous: Dict) -> bool:
    ...
    return any(current.get(field) != previous.get(field) for field in relevant_fields)
...
def generate_historical_data(
    lineups: List[Dict], existing_historical: List[Dict] = None
) -> List[Dict]:
    """
    Generate type 2 historical data from lineup scrapes.
    ...
    if existing_historical:
        for record in existing_historical:
            key = create_historical_key(
                record, record["festival_name"], record["festival_year"]
            )
    ...
    for lineup in sorted_lineups:
        ...
        for artist in lineup["artists"]:
            ...
            if "artist_id" not in artist:
                artist["artist_id"] = get_or_create_artist_id(
                    artist["artist_name"], artist_ids
                )
                    ...
                    new_record.update(
                        {
                            "festival_name": festival_name,
                            "festival_year": festival_year,
                            "_valid_from": latest_record[
                                "_valid_from"
                            ],  # Use original valid_from
                            "_valid_to": None,
                            "_is_current": True,
                            "artist_id": latest_record[
                                "artist_id"
                            ],  # Preserve the artist ID
                        }
                    )
    ...
    return [record for records in historical_records.values() for record in records]
...
def main():
    ...
    print(
        f"Added records: {len([r for r in historical_data if r['_valid_from'] == r['_valid_to']])}"
    )
    print(
        f"Removed records: {len([r for r in historical_data if r['_valid_to'] is not None])}"
    )
